...
- Workflow based on scriptable objects (save and load).
...
## Dialogue Editor
Edit the dialogue of a specific node by clicking on the *"Edit Dialogue"* button of the node. This will open a new window presenting an empty list of messages. Add a new message to the list. 
...
## Test the narrative
Before testing your narrative, open your narrative scriptable object's inspector. List all of the characters present in your narrative. 
...
Replaces the tag's position by a stored value named ```value```. The variable must be defined in the Scriptable Singleton "DialogueVariableData.asset". If not defined, the value will be ```"X"```.
